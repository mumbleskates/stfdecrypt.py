    ...
    if pkey.k_len == 2:
        for i in range(256):
            ...
            pkey.mk_tab[0][i] = pkey.m_tab[0][
                pkey.q_tab[0][pkey.q_tab[0][by] ^ byte(key[1], 0)] ^ byte(
                    key[0], 0)]
            pkey.mk_tab[1][i] = pkey.m_tab[1][
                pkey.q_tab[0][pkey.q_tab[1][by] ^ byte(key[1], 1)] ^ byte(
                    key[0], 1)]
            pkey.mk_tab[2][i] = pkey.m_tab[2][
                pkey.q_tab[1][pkey.q_tab[0][by] ^ byte(key[1], 2)] ^ byte(
                    key[0], 2)]
            pkey.mk_tab[3][i] = pkey.m_tab[3][
                pkey.q_tab[1][pkey.q_tab[1][by] ^ byte(key[1], 3)] ^ byte(
                    key[0], 3)]
    if pkey.k_len == 3:
        for i in range(256):
            by = i % 0x100
            pkey.mk_tab[0][i] = pkey.m_tab[0][pkey.q_tab[0][pkey.q_tab[0][
                                                                pkey.q_tab[1][
                                                                    by] ^ byte(
                                                                    key[2],
                                                                    0)] ^ byte(
                key[1], 0)] ^ byte(key[0], 0)]
            pkey.mk_tab[1][i] = pkey.m_tab[1][pkey.q_tab[0][pkey.q_tab[1][
                                                                pkey.q_tab[1][
                                                                    by] ^ byte(
                                                                    key[2],
                                                                    1)] ^ byte(
                key[1], 1)] ^ byte(key[0], 1)]
            pkey.mk_tab[2][i] = pkey.m_tab[2][pkey.q_tab[1][pkey.q_tab[0][
                                                                pkey.q_tab[0][
                                                                    by] ^ byte(
                                                                    key[2],
                                                                    2)] ^ byte(
                key[1], 2)] ^ byte(key[0], 2)]
            pkey.mk_tab[3][i] = pkey.m_tab[3][pkey.q_tab[1][pkey.q_tab[1][
                                                                pkey.q_tab[0][
                                                                    by] ^ byte(
                                                                    key[2],
                                                                    3)] ^ byte(
                key[1], 3)] ^ byte(key[0], 3)]
    if pkey.k_len == 4:
        for i in range(256):
            by = i % 0x100
            pkey.mk_tab[0][i] = (
                pkey.m_tab[0][pkey.q_tab[0][pkey.q_tab[0][
                                                pkey.q_tab[1][
                                                    pkey.q_tab[
                                                        1][
                                                        by] ^ byte(
                    ...
                    key[1], 0)] ^ byte(key[0], 0)])
            pkey.mk_tab[1][i] = (
                pkey.m_tab[1][pkey.q_tab[0][pkey.q_tab[1][
                                                pkey.q_tab[1][
                                                    pkey.q_tab[
                                                        0][
                                                        by] ^ byte(
                    ...
                    key[1], 1)] ^ byte(key[0], 1)])
            pkey.mk_tab[2][i] = (
                pkey.m_tab[2][pkey.q_tab[1][pkey.q_tab[0][
                                                pkey.q_tab[0][
                                                    pkey.q_tab[
                                                        0][
                                                        by] ^ byte(
                    ...
                    key[1], 2)] ^ byte(key[0], 2)])
            pkey.mk_tab[3][i] = (
                pkey.m_tab[3][pkey.q_tab[1][pkey.q_tab[1][
                                                pkey.q_tab[0][
                                                    pkey.q_tab[
                                                        1][
                                                        by] ^ byte(
...
def h_fun(pkey, x, key):
    ...
    if pkey.k_len >= 2:
        b0 = pkey.q_tab[0][pkey.q_tab[0][b0] ^ byte(key[1], 0)] ^ byte(key[0],
                                                                       0)
        b1 = pkey.q_tab[0][pkey.q_tab[1][b1] ^ byte(key[1], 1)] ^ byte(key[0],
                                                                       1)
        b2 = pkey.q_tab[1][pkey.q_tab[0][b2] ^ byte(key[1], 2)] ^ byte(key[0],
                                                                       2)
        b3 = pkey.q_tab[1][pkey.q_tab[1][b3] ^ byte(key[1], 3)] ^ byte(key[0],
                                                                       3)
    return (pkey.m_tab[0][b0] ^ pkey.m_tab[1][b1] ^ pkey.m_tab[2][b2] ^
            pkey.m_tab[3][b3])
...
def encrypt(pkey, in_blk):
    ...
    for i in range(8):
        t1 = (pkey.mk_tab[0][byte(blk[1], 3)] ^ pkey.mk_tab[1][
            byte(blk[1], 0)] ^ pkey.mk_tab[2][byte(blk[1], 1)] ^ pkey.mk_tab[3][
                  byte(blk[1], 2)])
        t0 = (pkey.mk_tab[0][byte(blk[0], 0)] ^ pkey.mk_tab[1][
            byte(blk[0], 1)] ^ pkey.mk_tab[2][byte(blk[0], 2)] ^ pkey.mk_tab[3][
                  byte(blk[0], 3)])
        ...
        t1 = (pkey.mk_tab[0][byte(blk[3], 3)] ^ pkey.mk_tab[1][
            byte(blk[3], 0)] ^ pkey.mk_tab[2][byte(blk[3], 1)] ^ pkey.mk_tab[3][
                  byte(blk[3], 2)])
        t0 = (pkey.mk_tab[0][byte(blk[2], 0)] ^ pkey.mk_tab[1][
            byte(blk[2], 1)] ^ pkey.mk_tab[2][byte(blk[2], 2)] ^ pkey.mk_tab[3][
                  byte(blk[2], 3)])
...
def decrypt(pkey, in_blk):
    ...
    for i in range(7, -1, -1):
        ...
        blk[3] = rotr32(
            blk[3] ^ ((t0 + 2 * t1 + pkey.l_key[4 * (i) + 11]) % 0x100000000),
            1)
